...
{
    // Convert the input words to uppercase
    ...
    // Find the pronunciation of each word in the dictionary
    auto it_word1 = dict.find(word1_upper);
    auto it_word2 = dict.find(word2_upper);

    // If both words are found in the dictionary
    if (it_word1 != dict.end() && it_word2 != dict.end()) {
        // Compare the pronunciations of the words as strings
        const string& pronunciation1 = vectorToString(it_word1->second);
        const string& pronunciation2 = vectorToString(it_word2->second);

        // Check if the pronunciations match
        if (pronunciation1 == pronunciation2) {
            return true; // Words are homophones
        }
    }

    return false; // Words are not homophones or one/both words not found in the dictionary
}

// Helper function to convert vector of strings to a single string
string PronounceDict::vectorToString(const vector<string>& vec) const {
    string result;
