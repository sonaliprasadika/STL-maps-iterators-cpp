...
void CommonWords::init_common() 
{
    for (const auto& file_map : file_word_maps) {
        for (const auto& word_count : file_map) {
            const string& word = word_count.first;
            // Increment count for the word if it exists, otherwise initialize it to 1
            common[word]++;
        }
    }

    // Filter out words that don't appear in all files
    ...
    // Debug: Print contents of common
    for (const auto& word_count : common) {
...
vector<string> CommonWords::get_common_words(unsigned int n) const
{
    vector<string> out;
    for (const auto& word_count : common) {
        if (word_count.second >= n && word_count.second == file_word_maps.size()) {
            out.push_back(word_count.first);
        }
    }
    return out;
}
